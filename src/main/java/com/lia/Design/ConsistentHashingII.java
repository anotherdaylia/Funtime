...
public class ConsistentHashingII {
    ...
    public List<Integer> addMachine(int machine_id) {
        ...
        for (int i = 1; i <= k; i++){
            int key = randomGenerator.nextInt(n);
            if (treeMap.containsKey(key)) {
                i--;
                continue;
            }
            treeMap.put(key, machine_id);
            shards.add(key);
        }

        //System.out.println("machineid: " + machine_id + ", " + shards.toString());
        return shards;
    }
    ...
    public int getMachineIdByHashCode(int hashcode) {
        ...
        Integer key = treeMap.ceilingKey(hashcode);
        if (key == null) {
            //System.out.println("id: " + treeMap.get(treeMap.firstKey()));
            return treeMap.get(treeMap.firstKey());
        } else {
            //System.out.println("id: " + treeMap.get(key));
